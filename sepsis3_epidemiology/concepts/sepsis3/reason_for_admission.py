...
This is essentially a rewrite of the script below, without requiring SQL at
all. See:
...
###############################################################################
# This section generatesthree tables:
# diagnosis_groups (level 0), diagnosis_subgroups (level 1) and
# diagnoses (level 2 – this is the main specialty diagnosis table)
...
# Itemids below are APACHE II and IV
...
# Typeid denotes the system providing the diagnoses groups (e.g. APACHE,
# legacy ICU)
...
# The idea is to retain only the most recent 'best' diagnoses group for each
# admission, where 'best' denotes the system type (typeid)
...
diagnosis_groups = diagnosis_groups.loc[(diagnosis_groups['rownum'] == 0)]
...
# This happens at the end of the SQL query
diagnosis_subgroups = diagnosis_subgroups.loc[
...
# This happens at the end of the SQL query
diagnoses = diagnoses.loc[(diagnoses['rownum'] == 0)]

###############################################################################
# Sepsis as explicitly recorded at admission

# Itemid 15808 is used to denote sepsis as recorded at admission by a clinician
...
# Note, this table contains only a subset of the admissions (those who been
# explicitly marked as having a positive or negative sepsis diagnosis).
# Patients who did not have this recorded will be absent in this table, but we
# later regard missing data here as False (i.e. no positive sepsis diagnosis).
# There are often multiple rows for the same admission. The purpose of rownum
# is to reorder for the final 'correct' entry, which is all we now retain.
sepsis = sepsis.loc[(sepsis['rownum'] == 0)]
...
# The following are not included (for the purpose of replicating Amsterdam's
# GitHub script). In other scripts, we make different decisions about
# antibiotics (in particular, metronidazol, co-trimoxazol, co-amoxiclav).
...
# This script is explicitly aimed at admission, so only retain antibiotics
# administered in the first 24hrs (only these are considered to contribute
# to the suspected infection on admission).
...
# The following are not included (similarly as for antibiotics - some of these
# could be included, but are not in order to reproduce Amsterdam's script):
...
# For the purposes of identifying sepsis at admission, a suspected infection
# requires the clinician to decide to order cultures within 6hrs of admission
...
diagnosis_groups_columns = ['admissionid', 'diagnosis_group']
diagnosis_groups_columns += ['diagnosis_group_id']
diagnosis_groups_add = diagnosis_groups[diagnosis_groups_columns]

diagnosis_subgroups_columns = ['admissionid', 'diagnosis_subgroup']
diagnosis_subgroups_columns += ['diagnosis_subgroup_id']
diagnosis_subgroups_add = diagnosis_subgroups[diagnosis_subgroups_columns]

diagnoses_columns = ['admissionid', 'diagnosis_type', 'diagnosis']
diagnoses_columns += ['diagnosis_id', 'surgical']
# This happens at the end of the SQL query
diagnoses_add = diagnoses[diagnoses_columns]

sepsis_add = sepsis[['admissionid', 'sepsis_at_admission']]
...
# This is a departure from the Amsterdam GitHub script (though followed a brief
# discussion with the authors of that work). In short, we assume the majority
# of these admissions are in fact emergency medical admissions (an original
# admission for surgical reasons may not lead to ICU admission for the same
# reasons, but for a secondary medical issue), unless there is explicit mention
# of surgical terms in the diagnoses.
...
combined_diagnoses['surgical'] = combined_diagnoses['surgical'].fillna(False)

# Convert these admissions to non-emergency surgical admissions
...
# Convert these admissions to emergency medical admissions
combined_diagnoses.loc[(
        (combined_diagnoses['urgency'] == 0) &
        (combined_diagnoses['surgical'] == 0)),
    'urgency'] = 1
...
# Note: 'x &= condition' is the same as 'x = (x & condition)', where & is the
...
# Or non-surgical respiratory failure
combined_diagnoses['respfailure'] |= (
    (combined_diagnoses['surgical'] == 0) &
    (combined_diagnoses['diagnosis'].str.contains(
...
###############################################################################
# This is unused, but potentially useful


def counts_fun(df):
    '''Return the number of diagnoses in each diagnosis group.'''
    columns = ['diagnosis_group', 'diagnosis']
    counts = df.groupby(columns).size().to_frame('n_admissions')
...
# This includes 'MC&IC' and 'IC&MC' (i.e. admissions that include partial
# ICU admission/transfer to ICU from MCU)
icu_ind = combined_diagnoses['location'].str.contains('IC')
# Note, the symbols either side of 'MC' in '^MC$' explicitly do not allow
# 'MC&IC' and 'IC&MC'
...
reason_columns = ['Distinct patients', 'ICU admissions', 'Surgical admissions']
reason_columns += ['Medical admissions']
reason_columns += ['Urgent admissions', 'Cardiothoracic', 'Sepsis']
reason_columns += ['Respiratory failure', 'Neurosurgery', 'Trauma']
reason_columns += ['GI surgery', 'Vascular surgery', 'Cardiac arrest']
reason_columns += ['Neurological', 'Cardiac disorder']
reason_for_admission = pd.DataFrame(
    index=reason_columns, columns=['Total', 'ICU', 'MCU'])
...
column_dict = {
    ...
    'Trauma': 'trauma',
    'GI surgery': 'gisurg',  # renamed from Amsterdam's script
...
########################
# Original SQL call from Amsterdam's script, which has been rewritten above
