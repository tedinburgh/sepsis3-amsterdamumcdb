...
file_path = '../../data/additional_files/'
...
procedureorderitems = pd.read_csv(file_path + 'procedureorderitems.csv', \
...
diagnosis_groups = diagnosis_groups.sort_values(
    by=['admissionid', 'typeid', 'measuredat'], ascending=False)
...
diagnosis_subgroups = diagnosis_subgroups.sort_values(
    by=['admissionid', 'measuredat'], ascending=False)
...
diagnoses.loc[
        diagnoses['itemid'].isin([18669]) & \
        (diagnoses['diagnosis_id'] >= 1) & (diagnoses['diagnosis_id'] <= 26),
    'surgical'] = 1
diagnoses.loc[
        diagnoses['itemid'].isin([18671]) & \
        (diagnoses['diagnosis_id'] >= 222) & (diagnoses['diagnosis_id'] <= 452),
    'surgical'] = 1
##
...
diagnoses.loc[
        (diagnoses['itemid'] >= 16998) & (diagnoses['itemid'] <= 17017),
    'typeid'] = 4
diagnoses.loc[
        (diagnoses['itemid'] >= 18589) & (diagnoses['itemid'] <= 18602),
    'typeid'] = 3
diagnoses.loc[
        (diagnoses['itemid'] >= 13116) & (diagnoses['itemid'] <= 13145),
    'typeid'] = 2
diagnoses.loc[
        (diagnoses['itemid'] >= 16642) & (diagnoses['itemid'] <= 16673),
    'typeid'] = 1
...
sepsis = sepsis.groupby(['admissionid', 'measuredat']).agg(
        sepsis_at_admission=pd.NamedAgg(
            column='sepsis_at_admission', aggfunc='any')
    ).reset_index()
sepsis = sepsis.sort_values(by=['admissionid', 'measuredat'], ascending=False)
...
combined_diagnoses.to_csv(file_path + 'combined_diagnoses.csv', index=False)

################################################################################
## Also compute all the reasons for admission for comparison to AmsterdamUMCdb
## script.

icu_ind = admissions_df['location'].str.contains('IC')
mcu_ind = admissions_df['location'].str.contains('^MC$')

## Number of patients
no_patients_total = admissions_df['patientid'].nunique()
no_patients_ICU = admissions_df.loc[icu_ind, 'patientid'].nunique()
no_patients_MCU = admissions_df.loc[mcu_ind, 'patientid'].nunique()

## Number of admissions
no_admissions_total = admissions_df['admissionid'].nunique()
no_admissions_ICU = admissions_df.loc[icu_ind, 'admissionid'].nunique()
no_admissions_MCU = admissions_df.loc[mcu_ind, 'admissionid'].nunique()
...
cardiosurg_counts = counts_fun(cardiosurg)
cardiosurg_counts.head()

combined_diagnoses['cardiosurg'] = combined_diagnoses['admissionid'].isin(
    cardiosurg['admissionid'])
...
respfailure_counts = counts_fun(respfailure)
respfailure_counts.head()

combined_diagnoses['respfailure'] = combined_diagnoses['admissionid'].isin(
    respfailure['admissionid'])
...
neurosurg_counts = counts_fun(neurosurg)
neurosurg_counts.head()

combined_diagnoses['neurosurg'] = combined_diagnoses['admissionid'].isin(
    neurosurg['admissionid'])
...
gisurg_counts = counts_fun(gisurg)
gisurg_counts.head()

combined_diagnoses['gisurg'] = combined_diagnoses['admissionid'].isin(
    gisurg['admissionid'])
...
cardiacarrest_counts = counts_fun(cardiacarrest)
cardiacarrest_counts.head()

combined_diagnoses['cardiacarrest'] = combined_diagnoses['admissionid'].isin(
    cardiacarrest['admissionid'])
...
vascsurg = combined_diagnoses[
    ...
    ~(combined_diagnoses['diagnosis_group'].str.contains(
        'neuro', na=False, flags=re.IGNORECASE))]

vascsurg_counts = counts_fun(vascsurg)
vascsurg_counts.head()

combined_diagnoses['vascsurg'] = combined_diagnoses['admissionid'].isin(
    vascsurg['admissionid'])
...
trauma_counts = counts_fun(trauma)
trauma_counts.head()

combined_diagnoses['trauma'] = combined_diagnoses['admissionid'].isin(
    trauma['admissionid'])
...
neuro_counts = counts_fun(neuro)
neuro_counts.head()

combined_diagnoses['neuro'] = combined_diagnoses['admissionid'].isin(
    neuro['admissionid'])
...
cardio_counts = counts_fun(cardio)
cardio_counts.head()

combined_diagnoses['cardio'] = combined_diagnoses['admissionid'].isin(
    cardio['admissionid'])
